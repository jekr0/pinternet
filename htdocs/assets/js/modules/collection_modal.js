class CollectionModalComponent {
  ...
  async load(){
    ...
    const addItem=document.createElement('li'); const addButton=document.createElement('button'); addButton.type='button'; addButton.className='collection-modal__collection-item collection-modal__collection-item--add'; addButton.setAttribute('aria-label','Редактировать коллекции'); addButton.innerHTML='<span class="collection-modal__edit-icon" data-svg-src="/assets/images/icons/S-edit.svg" aria-hidden="true"></span>'; App.utils.loadSVG('/assets/images/icons/S-edit.svg',addButton.querySelector('[data-svg-src]')); addButton.addEventListener('click',()=>{if(this.mode==='create'){this.nameInput?.focus();return;} this.setCreateMode(true); this.nameInput?.focus();}); addItem.appendChild(addButton); this.list.appendChild(addItem);
  }
