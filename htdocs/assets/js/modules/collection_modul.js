class CollectionModalComponent {
  ...
  alignTagRows(){ if(!this.tagsList) return; const rows=Array.from(this.tagsList.querySelectorAll('.collection-modal__tags-row')); const availableWidth=this.tagsList.clientWidth||370; rows.forEach((r)=>{const chips=Array.from(r.querySelectorAll('.collection-modal__tag-item')); if(chips.length<=1){r.style.justifyContent='flex-start';r.style.columnGap='8px';return;} const hasMore=chips.some((c)=>c.classList.contains('collection-modal__tag-item--more')); const rowWidth=chips.reduce((sum,c)=>sum+(c.offsetWidth||36),0)+8*(chips.length-1); if(hasMore||rowWidth>=availableWidth-4){r.style.justifyContent='space-between';r.style.columnGap='0px';} else {r.style.justifyContent='flex-start';r.style.columnGap='8px';}}); }
